...
option_list <- list(
  ...
  # seqInfo added by Ivna
  make_option(c("--seqInfo"), type="character", default=NULL, help="Path to chromosome sizes txt file. Required."),
...
## load seqinfo
## Changed by Ivna: LeoMed doesn't have access to internet and this file cannot be automatically downloaded
## seqinfo <- getSeqInfo(genomeBuild, genomeStyle)
## TODO: argpase from config file
seq <- read.table(seqinfo_file)
# seq <- read.table("scripts/data/chromInfo_hg38.txt")
seqnames <- seq$V1
seqlengths <- seq$V2
genome <- "hg38"
seqinfo <- Seqinfo(seqnames, seqlengths, isCircular=rep(FALSE, length(seqnames)), genome)
